...
import com.carsecommerce.admin.order.OrderRepository;
...
public class MasterOrderReportService {
	...
	private void printReportData(List<ReportItem> listReportItems) {
		listReportItems.forEach(item -> {
			System.out.printf("%s, %10.2f, %d \n", item.getIdentifier(), item.getGrossSales(), item.getOrdersCount());
		});
	...
	private void printRawData(List<Order> listOrders) {
		listOrders.forEach(order -> {
			System.out.printf("%-3d | %s | %10.2f \n", order.getId(), order.getOrderTime().toString(), order.getTotal());
		});
	}
}